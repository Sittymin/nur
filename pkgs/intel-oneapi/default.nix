...
stdenv.mkDerivation rec {
  ...
  installPhase = ''
    ...
    find $out/tmp/l_BaseKit_p_${version}_offline/lib -print -type f
    for files in `find $out/tmp/l_BaseKit_p_${version}_offline/lib -type f`
    do
      patchelf --set-rpath "${glibc}/lib:$libPath:$out/tmp/l_BaseKit_p_${version}_offline/lib" $files
    done
    ...
    $out/tmp/l_BaseKit_p_${version}_offline/install.sh --action downloadonly --components all --log-dir $out/log --eula accept -s
  ...
  postFixup = ''
    ...
    do
      ...
      do
          echo "       $file"
          patchelf --set-interpreter "$(cat $NIX_CC/nix-support/dynamic-linker)" --set-rpath '$ORIGIN'":${glibc}/lib:$libPath:$dir/latest/lib64:$out/${version_dir}/lib" $file
      done
    done
  '';
